...
/* page_ptr */
page_ptr::page_ptr(intptr_t x){
    this->page_id = (uint32_t) x ;
    this->offset =  x>>32;
}
page_ptr::operator void*() const {

    assert(pm!=nullptr);
    // if(!pm) return nullptr;
    // printf("get void*, page_id:%d, offset:%d\n", this->page_id, this->offset);
    if(!pm->page_index[this->page_id]){
        panic("Page is not existed\n");
        // return nullptr 
    }

    page* pg = pm->page_index[this->page_id];
    if(!pg->is_mapped) {
        printf("[DEBUG] page:%d is not mapped\n", pg->ph->page_id);
        if(map_page(pg, pg->path)==-1){
            panic("Map page failed\n");
        }
    }
    return (char*)pg->ph+this->offset;
    // return nullptr;
}
page_ptr::operator intptr_t() const{
    return ((intptr_t)offset << 32) + page_id;
}
/*API*/
#define PAGE_HEAD pg->ph
page_ptr psmalloc(size_t size){
    return psmalloc(size, PAGE_TYPE_CHUNK);
}
page_ptr psmalloc(size_t size, int page_type) {
    ...
    if(page_type == PAGE_TYPE_MAP || size > MAX_CHUNK_SIZE) {
        ...
        extend_page_index();
        sprintf(new_page_path,"%s/%d.pg", DEFAULT_PAGE_DIR, pm->cur_page_id);
        page_head* new_page_head = _create_page(new_page_path, 0, pm->cur_page_id, size);
        if(!new_page_head){
    ...
    if(pm->free_list[idx].page_id==0) {
        ...
        new_pg->is_mapped = true;
        pm->page_index[new_pg->ph->page_id] = new_pg;
        pm->free_list[idx] = {new_pg->ph->page_id, sizeof(page_head)};
        printf("[DEBUG]: create new page, %s\n",new_pg->path);
    ...
    /*map page */
    page* pg = pm->page_index[pm->free_list[idx].page_id];
    // printf("[DEBUG] search page:%d from page_index\n", PAGE_HEAD->page_id);
    if(!pg->is_mapped) {
    ...
    page_ptr p =  {pm->free_list[idx].page_id, pm->free_list[idx].offset};
    pm->free_list[idx] = (page_ptr)*(intptr_t*)(void*)p;
    return p;
}
...
void psfree(page_ptr ptr) {
    ...
    page* pg = pm->page_index[ptr.page_id];
    assert(pg!=nullptr);
    ...
    *(intptr_t*)(void*)ptr = (intptr_t)pm->free_list[idx];
...
int page_manager::init(const char* page_entry_name) {
    ...
    pm->free_list = (page_ptr*)(ph+1);
    printf("[DEBUG] cur_page_id:%d\n",pm->cur_page_id);
...
static page_head* _create_page(const char* filepath, int chunk_size, int page_id, int page_size)
{
    ...
    for (int chunk_index = 0; chunk_index<chunk_num - 2; chunk_index++) {
        *(intptr_t*)(chunk_current+(intptr_t)ph) = ((intptr_t)chunk_next<<32) + page_id;
